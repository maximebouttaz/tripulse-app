...
export default async function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-zinc-950">
      {/* ── Sidebar ── */}
      <aside className="hidden md:flex w-64 flex-col fixed inset-y-0 left-0 z-40 bg-zinc-900 border-r border-zinc-800">
        {/* Brand */}
        <div className="flex items-center gap-2.5 px-6 h-16 border-b border-zinc-800 shrink-0">
          ...
          <span className="ml-auto text-[10px] font-bold text-zinc-600 uppercase tracking-wider">
            Pro
          </span>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {NAV_LINKS.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              className="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all"
            >
        ...
        <div className="shrink-0 border-t border-zinc-800 p-4 space-y-3">
          ...
          <form action="/auth/signout" method="post">
            <button
              type="submit"
              className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-zinc-500 hover:text-white hover:bg-zinc-800 transition-all"
            >
              <LogOut size={16} />
              Se déconnecter
            </button>
      ...
      {/* ── Mobile top bar ── */}
      <div className="md:hidden fixed top-0 inset-x-0 z-50 h-14 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between px-4">
